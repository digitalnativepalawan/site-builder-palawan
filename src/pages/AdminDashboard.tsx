...
export default function AdminDashboard() {
  ...
  const parsed: ParsedSubmission[] = (submissions || []).map((sub: any) => {
    ...
    return {
      ...
      resortName, resortOwner, sectionCount, sectionPct,
      ...
      <div className="max-w-5xl mx-auto">
        <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            {draftCount > 0 && (
              <button onClick={() => setConfirmingPurge(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold transition-all border border-destructive/30 text-destructive hover:bg-destructive/10">
                <RotateCcw className="w-3.5 h-3.5" /> Purge {draftCount} Drafts
              </button>
            )}
            <button onClick={() => setHideDrafts(!hideDrafts)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold transition-all border ${hideDrafts ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/40"}`}>
